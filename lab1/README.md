...
Enter _AmazonEC2ContainerRegistryFullAccess_ in the Filter text field.

![ECSContainerRegistryFullAccess](./img/3.png)

Click **Next.**

Enter _EcsLabWorkstationProfile_ for the Role name and click **Create Role**.
...
In the Create Role screen, enter _AmazonEC2ContainerServiceforEC2Role AmazonEC2ContainerServiceAutoscaleRole_ in the text field (without a comma) and select the two policies.

![Select two policies](./img/5.png)

In the Review screen, enter _EcsLabInstanceProfile_ for the Role name and click **Create Role**.
...
Create a new security group by navigating to [the EC2 console > Security Group](https://console.aws.amazon.com/ec2/v2/home?region=us-east-1#SecurityGroups:sort=tag:Name) and create a Security Group named  _SGEcsLabPublicCluster_. Keep the defaults.  Make sure the correct VPC is selected when creating the security group.
...
You should not be prompted for sudo access.
...
<html> 
<head></head>
<body>hi! I'm served via Python + Flask. I'm a web endpoint. </body>
</html>
```
...
To tag and push to the web repository (if you're using a shared account, use your name in the tag e.g. `fred-ecs-lab:latest`):

```
$ docker tag ecs-lab/web:latest <account-id>.dkr.ecr.us-east-1.amazonaws.com/ecs-lab-web:latest

$ docker push <account-id>.dkr.ecr.us-east-1.amazonaws.com/ecs-lab-web:latest
```

This should return something like this:

The push refers to a repository [\<account-id\>.ecr.us-east-1.amazonaws.com/ecs-lab-web] (len: 1)
...
Digest: sha256:fa0601417fff4c3f3e067daa7e53.....
```

To tag and push to the api repository:

```
$ docker tag ecs-lab/api:latest <account-id>.dkr.ecr.us-east-1.amazonaws.com/ecs-lab-api:latest

$ docker push <account\-id>.dkr.ecr.us-east-1.amazonaws.com/ecs-lab-api:latest
```
...

{
    "imageIds": [
        {
            "imageTag": "latest", 
            "imageDigest": "sha256:54eae66be5e2bf910bb5ac...."
        }
    ]
}
```
